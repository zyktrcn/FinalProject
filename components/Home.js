...
export default class Home extends Component<Props> {
  ...
  static navigationOptions = {
    tarBarLabel: 'Home',
    tabBarIcon: () => {
      return <Image style={styles.tabBarIcon} source={require('../images/home.png')} />
    }
  }
  render() {
    return (
      <View style={styles.container}>
        <ScrollView>
          <BannerSwiper />
          <Text style={styles.title}>计算机</Text>
          <View style={styles.items}>
            {
              this.state.new.map((el, index) => {
                if (index < 2) {
                  return (<Item item={el} navigation={ this.props.navigation } />)
                }
              })
            }
          </View>
          ...
          <Text style={styles.title}>外语</Text>
          <View style={styles.items}>
            {
              this.state.old.map((el, index) => {
                if (index < 2) {
                  return (<Item item={el} navigation={ this.props.navigation } />)
                }
              })
